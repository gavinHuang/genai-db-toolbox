...
class DataExtractor:
    ...
    def extract_data_model(self, data_limit: int = 10000) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Data model extraction complete: {results['extraction_info']['total_tables']} tables, "
            ...
            for table_name in table_names:
                ...
                try:
                    ...
                    if table_df is not None and not table_df.empty:
                        ...
                        self.logger.debug(f"Extracted table '{table_name}': {table_info['row_count']} rows, {table_info['column_count']} columns")
                    else:
    ...
    def _analyze_column(self, col_name: str, col_data, full_col_data) -> Dict[str, Any]:
        ...
        col_info = {
            ...
            "null_percentage": float(null_count / len(full_col_data) * 100) if len(full_col_data) > 0 else 0,
    ...
    def _detect_data_category(self, col_name: str, col_data) -> str:
        ...
        if any(pattern in col_name_lower for pattern in ['id', 'key', 'code']):
            ...
            return 'Date/Time'
        elif any(pattern in col_name_lower for pattern in ['amount', 'price', 'cost', 'value', 'revenue']):
            return 'Currency'
            ...
            if hasattr(pbix, 'relationships') and pbix.relationships is not None:
                ...
                if hasattr(rel_data, 'iterrows'):
                    ...
                    for _, row in rel_data.iterrows():
                        relationship = {
                            "from_table": row.get(columns[0] if columns else 'FromTable', 'Unknown'),
                            "from_column": row.get(columns[1] if len(columns) > 1 else 'FromColumn', 'Unknown'),
                            "to_table": row.get(columns[2] if len(columns) > 2 else 'ToTable', 'Unknown'),
                            "to_column": row.get(columns[3] if len(columns) > 3 else 'ToColumn', 'Unknown'),
                            "cardinality": row.get(columns[4] if len(columns) > 4 else 'Type', 'One-to-Many'),
                            "is_active": row.get(columns[5] if len(columns) > 5 else 'IsActive', True),
            ...
            if hasattr(pbix, 'power_query') and pbix.power_query is not None:
                ...
                if hasattr(pq_data, 'iterrows'):
                    ...
                    for _, row in pq_data.iterrows():
                        ...
                        query_expression = row.get(columns[2] if len(columns) > 2 else 'Expression', '')
                ...
                if hasattr(pbix.dax_tables, 'empty'):  # It's a DataFrame
                    ...
                    for table_name, table_info in pbix.dax_tables.items():
                        if isinstance(table_info, dict):
                            expression = table_info.get('expression', table_info.get('Expression', ''))
                            calculated_tables.append({
                                "name": table_name,
                                "expression": expression,
                                "description": table_info.get('description', table_info.get('Description', '')),
                                "is_hidden": table_info.get('isHidden', table_info.get('IsHidden', False)),
                                "expression_length": len(str(expression)),
                                "function_count": self._count_dax_functions(str(expression)),
                                "complexity_score": self._calculate_dax_complexity(str(expression)),
                                "referenced_tables": self._extract_referenced_tables(str(expression))
    ...
    def _count_dax_functions(self, expression: str) -> int:
        ...
        dax_functions = [
            ...
            'DATEADD', 'DATEDIFF', 'TOTALYTD', 'TOTALQTD', 'TOTALMTD',
            'RELATED', 'RELATEDTABLE', 'LOOKUPVALUE',
            'IF', 'SWITCH', 'AND', 'OR', 'NOT',
            'FORMAT', 'CONCATENATE', 'SUBSTITUTE'
        ]
    ...
    def _contains_time_intelligence(self, expression: str) -> bool:
        ...
        time_functions = [
            ...
            'DATEADD', 'DATEDIFF', 'PARALLELPERIOD',
            'SAMEPERIODLASTYEAR', 'PREVIOUSMONTH', 'NEXTMONTH',
