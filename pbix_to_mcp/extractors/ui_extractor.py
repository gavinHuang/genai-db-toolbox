...
class UIExtractor:
    ...
    def extract_ui_structure(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"UI structure extraction complete: {ui_data['extraction_info']['total_pages']} pages, {ui_data['extraction_info']['total_visuals']} visuals")
            return ui_data
    ...
    def _parse_single_page(self, page_data: Dict, ordinal: Optional[int] = None) -> Optional[Dict[str, Any]]:
        """Parse a single report page."""
        if not isinstance(page_data, dict):
            return None
        
        # Extract page name
        page_name = page_data.get("name", f"Page_{ordinal if ordinal is not None else page_data.get('ordinal', 'Unknown')}")
    ...
    def _parse_single_visual(self, visual_data: Dict, path: str, page_context: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            enhanced_type = self.VISUAL_TYPE_MAP.get(visual_type, visual_type.title() if visual_type else "Unknown")
    ...
    def _extract_custom_visual(self, pbix_zip: zipfile.ZipFile, visual_file: str) -> Optional[Dict[str, Any]]:
        """Extract custom visual information."""
        try:
