...
class PBIXConverter:
    ...
    def __init__(self, pbix_path: str, output_dir: Optional[str] = None):
        ...
        self.logger = setup_logger(
            name=f"pbix_converter_{self.pbix_name}",
    ...
    def extract_all(self, 
                   ...
                   data_limit: int = 10000) -> Dict[str, Any]:
        """
    ...
    def _generate_documentation(self) -> str:
        ...
        for table in self.extraction_results.get("data_model", {}).get("tables", []):
            doc_lines.append(f"- `{table.get('name', 'unknown')}`: {len(table.get('columns', []))} columns")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "dax_measures": len(self.extraction_results.get("dax_expressions", {}).get("measures", [])),
            "ui_pages": len(self.extraction_results.get("ui_structure", {}).get("pages", [])),
            "visualizations": len(self.extraction_results.get("ui_structure", {}).get("visualizations", [])),
