...
class SQLiteGenerator:
        ...
        try:
            ...
            try:
                ...
                if "data_model" in extraction_results:
                    ...
                    self._create_comprehensive_metadata_tables(cursor, extraction_results["data_model"])
    ...
    def _create_data_tables(self, cursor: sqlite3.Cursor, data_model: Dict[str, Any]) -> None:
        ...
        for table in tables:
            if not table.get("data_extracted") or table.get("data") is None:
                self.logger.warning(f"Skipping table {table.get('name', 'Unknown')} - no data extracted")
                continue
            ...
            try:
                ...
                table_df.to_sql(safe_table_name, cursor.connection, index=False, if_exists='replace')
                
                self.logger.info(f"Created data table: {safe_table_name} ({len(table_df)} rows, {len(table_df.columns)} columns)")
    ...
    def _create_comprehensive_metadata_tables(self, cursor: sqlite3.Cursor, data_model: Dict[str, Any]) -> None:
    ...
    def _create_metadata_table(self, cursor: sqlite3.Cursor, extraction_results: Dict[str, Any]) -> None:
